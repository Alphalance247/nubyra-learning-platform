...
//             <span className="text-sm text-[#120A02] truncate max-w-[180px]">{file.name}</span>
//           </div>
//           <button
...
//       <small className="text-gray-500">Choose compressed or zip files less than 10 MB</small>
//     </div>
//   );
// };
...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      const isValidType = accept
        .split(',')
        .some(type => selectedFile.name.toLowerCase().endsWith(type.trim().toLowerCase()));
  ...
  return (
    <div className={`w-full max-w-md flex flex-col gap-2 ${className}`}>
      <label htmlFor={name} className="font-inter font-medium text-[16px] leading-[24px] text-[#120A02]">
        {label} {required && <span className="text-red-500">*</span>}
      </label>
      ...
      {!file ? (
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-[#FEFEFD] rounded-[12px] border border-[#D1D1D1]">
          <div className="flex items-center gap-3">
            <img src="https://upload.wikimedia.org/wikipedia/commons/8/87/PDF_file_icon.svg" alt="File icon" className="w-6 h-6" />
            <span className="text-sm text-[#120A02] truncate max-w-[180px]">{file.name}</span>
          </div>
          <button onClick={handleRemove} className="flex items-center text-sm text-red-600 hover:text-red-800">
            Remove <X size={14} className="ml-1" />
          </button>
        </div>
      )}
